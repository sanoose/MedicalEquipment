...
import SupplyOrderDetails from '@/Components/Common/SupplyOrderDetails.vue'
...
const form = useForm({
  ...
  supply_order_status: props.filters.supply_order_status || '',
...
const ApplyFilters = () => {
  form.get(route('SuppliesOrders.index'), {
    preserveState: true,
    preserveScroll: true,
    replace: true,
  })
...
/* ====== Status helpers ====== */
const StatusLabel = (v) => constants.supply_order_status?.[String(v)] || '-'
const StatusClass = (v) => {
  const s = String(v)
...
const ChangeStatus = (o, supply_order_status) => {
  const title = StatusLabel(supply_order_status)
  ...
  swal.fire({
    ...
    html: `
      ...
      هل أنت متأكد؟
    `,
    icon: supply_order_status == 3 ? 'warning' : 'info',
    showCancelButton: true,
    confirmButtonColor: supply_order_status == 2 ? 'green' : supply_order_status == 3 ? 'red' : 'yellowgreen',
    cancelButtonColor: 'gray',
    cancelButtonText: 'إلغاء الأمر',
    confirmButtonText: 'نعم',
    preConfirm: () => {
      router.visit(route('SuppliesOrders.ChangeStatus', { id: o.id, supply_order_status }), {
        method: 'get',
        preserveState: true,
        preserveScroll: true,
  ...
  <div class="mx-auto px-4 py-8 w-full" dir="rtl">
    ...
    <form class="bg-white p-6 rounded-xl shadow-md mb-8 border border-gray-200" @submit.prevent="ApplyFilters">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
        <div>
          ...
          <input
            v-model="form.search"
            type="text"
            placeholder="اسم المنشأة / الجهة أو اسم المسؤول..."
      ...
      <div class="flex space-x-3">
        <button class="bg-[#c5b26b] hover:bg-[#b3a15f] text-white font-medium py-2 px-6 rounded-lg transition duration-300 transform hover:-translate-y-0.5 hover:shadow-md">
          بحث
        </button>
      </div>
    </form>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(o, index) in orders.data" :key="o.id" class="hover:bg-gray-50 transition">
              ...
              <td class="px-6 py-4 text-sm text-gray-700">
                ...
                <div class="text-xs text-gray-500">{{ o.client?.client_name || '-' }}</div>
              </td>

              <td class="px-6 py-4 text-sm text-gray-700">{{ o.order_date || '-' }}</td>
              ...
              <td class="px-6 py-4">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="StatusClass(o.supply_order_status)">
                  {{ StatusLabel(o.supply_order_status) }}
                </span>
              </td>
                ...
                <div class="relative inline-block text-left pl-[70px]"  >
                  ...
                  >
                    <ul class="py-1 text-sm text-gray-700">
                      ...
                      <li>
                        <button v-if="o.supply_order_status != 2" @click="ChangeStatus(o, 2)" class="w-full text-right px-4 py-2 hover:bg-gray-100">
                          <fa class="ml-1 text-green-500" :icon="'check'" /> قبول
                        </button>
                      </li>

                      <li>
                        <button v-if="o.supply_order_status != 3" @click="ChangeStatus(o, 3)" class="w-full text-right px-4 py-2 hover:bg-gray-100">
                          <fa class="ml-1 text-red-500" :icon="'xmark'" /> رفض
                        </button>
                      </li>

                      <li>
                        <button v-if="o.supply_order_status != 1" @click="ChangeStatus(o, 1)" class="w-full text-right px-4 py-2 hover:bg-gray-100">
